...
class StatisticsHandler:
    ...
    def register(self):
        ...
        async def handle_main_statistics_button(message: types.Message):
            await message.answer("Выберите тип аналитики:", reply_markup=Keyboards.get_all_statistics_buttons())
            
        @self.router.callback_query(F.data.in_(["per_day", "per_week", "per_month", "per_year"]))
        async def handle_statistics_per_period(callback: types.CallbackQuery):
            ...
            statistics_dto = StatisticsRequest(callback.from_user.id, period_type)
